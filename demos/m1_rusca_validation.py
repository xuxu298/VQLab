...
Two modelling notes (kept honest):
  ...
  * Dead time (the figure's low-loss plateau) is folded into the SKR via ChannelParams.tau_dead
    -> a duty factor on the detection rate, the same mechanism the engine detector applies.
...
REP_RATE = 1e9
# Y0 = 2*p_dc = 1e-8; dead time folded into the SKR via tau_dead (-> duty factor).
...
def main() -> None:
    ...
    print(f"  R={REP_RATE:.0e} Hz, eta={PARAMS.eta_det}, Y0=1e-8, e_d={PARAMS.e_d}, "
          f"f_EC={F_EC}, dead time={PARAMS.tau_dead*1e9:.0f} ns\n")

    fig, ax = plt.subplots(figsize=(7.5, 5))
    for nZ in BLOCKS:
        skr = sweep(nZ, losses)
        reach = losses[skr > 0].max() if np.any(skr > 0) else 0.0
        skr26 = float(np.interp(26.0, losses, skr))
        print(f"  n_Z={nZ:.0e}:  reach={reach:.0f} dB   SKR@26dB={skr26:.3e} Hz")
        m = skr > 0
        ax.semilogy(losses[m], skr[m], "o-", ms=3.5, color=COLORS[nZ],
